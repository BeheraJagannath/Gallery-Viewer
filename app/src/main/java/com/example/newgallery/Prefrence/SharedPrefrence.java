...
public class SharedPrefrence {
    ...
    public static ArrayList<RecyclerModel> getRecycleBinData(Context c1) {
        SharedPreferences sharedpreferences = c1.getSharedPreferences(MyPREFERENCES, Context.MODE_PRIVATE);
        Gson gson = new Gson();
        String response = sharedpreferences.getString(RECYCLE_LIST, "");
        ArrayList<RecyclerModel> lstArrayList = gson.fromJson(response,
                new TypeToken<List<RecyclerModel>>() {
                }.getType());
    ...
    public static void setRecycleBinData(Context c1, ArrayList<RecyclerModel> hideFileList) {
        Gson gson = new Gson();
